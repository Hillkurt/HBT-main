  ...
  return (
    <>
      ...
      <aside className={clsx(
        "w-64 bg-[var(--primary)] text-white flex flex-col fixed left-0 top-0 shadow-2xl transition-all duration-300 z-50",
        "h-[100dvh] max-h-[100dvh]",
        // Mobilde: isOpen false ise sola kaydır (gizle)
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {visibleItems.map((item) => (
            <NavLink
              key={item.name}
              to={item.path}
              onClick={onClose} // Mobilde menü öğesine tıklayınca sidebar'ı kapat
              className={({ isActive }) =>
                clsx(
        ...
        <div className="p-4 border-t border-white/10 flex items-center" style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}>
